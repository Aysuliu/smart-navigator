...
class WeatherTourismService {
  ...
  const WeatherSmartRecommendation = ({ attraction }) => {
    const { weather, loading, getRecommendation } = useWeatherRecommendation();
    const [recommendation, setRecommendation] = useState(null);
    const [recLoading, setRecLoading] = useState(false);
  
    useEffect(() => {
      if (attraction && weather) {
        const fetchRecommendation = async () => {
          setRecLoading(true);
          const rec = await getRecommendation(attraction);
    ...
    }, [attraction, weather, getRecommendation]);
  
    if (loading || recLoading) {
      return <div className="animate-pulse">Loading weather recommendation...</div>;
    }
    ...
    return (
      <div className={`p-4 rounded-lg border-l-4 ${
        rec.status === 'excellent' ? 'bg-green-50 border-green-500' :
        rec.status === 'good' ? 'bg-yellow-50 border-yellow-500' :
        rec.status === 'poor' ? 'bg-red-50 border-red-500' :
        'bg-gray-50 border-gray-500'
      }`}>
        <div className="flex items-center mb-2">
          <span className="text-2xl mr-2">{rec.icon}</span>
          <h3 className="text-lg font-semibold">{rec.message}</h3>
        </div>
        ...
        {rec.activities && (
          <div className="mb-3">
            <h4 className="font-medium text-sm text-gray-600 mb-1">Recommended Activities:</h4>
        ...
        {rec.tips && (
          <div className="mb-3">
            <h4 className="font-medium text-sm text-gray-600 mb-1">Tips:</h4>
        ...
        {rec.alternative && (
          <div className="mt-3 p-3 bg-white rounded border">
            <div className="font-medium text-blue-600 mb-1">💡 Alternative Suggestion:</div>
            <div className="text-sm">{rec.alternative}</div>
          </div>
        )}
        
        <div className="mt-3 flex items-center text-sm">
          <span className="font-medium">Expected Crowd Level:</span>
          <span className={`ml-2 px-2 py-1 rounded text-xs ${
            rec.crowdMultiplier > 1.3 ? 'bg-red-200 text-red-800' :
            rec.crowdMultiplier < 0.7 ? 'bg-green-200 text-green-800' :
            'bg-yellow-200 text-yellow-800'
          }`}>
            {rec.crowdMultiplier > 1.3 ? 'Higher crowds due to weather' :
             rec.crowdMultiplier < 0.7 ? 'Lower crowds due to weather' :
             'Normal crowd levels'
            }
          </span>
        </div>
  ...
  // Integration with main crowd detection system
  const integrateCrowdAndWeather = async (attraction, crowdData, weatherService) => {
    const weather = await weatherService.fetchWeatherData();
    const weatherRec = weatherService.generateWeatherRecommendation(attraction, weather);
    
    // Combine crowd data with weather impact
    const adjustedCrowdLevel = crowdData.percentage * weatherRec.recommendation.crowdMultiplier;
    
    const finalRecommendation = {
      attraction: attraction.name,
      crowd: {
        baseLevel: crowdData.percentage,
        weatherAdjusted: Math.round(adjustedCrowdLevel),
        status: adjustedCrowdLevel > 80 ? 'very_crowded' : 
                adjustedCrowdLevel > 60 ? 'crowded' : 
                adjustedCrowdLevel > 40 ? 'moderate' : 'quiet'
      },
      weather: weatherRec.recommendation,
      finalMessage: generateFinalMessage(adjustedCrowdLevel, weatherRec.recommendation),
      recommendVisit: adjustedCrowdLevel < 70 && weatherRec.recommendation.status !== 'poor'
    };
    
    return finalRecommendation;
  };
  
  const generateFinalMessage = (crowdLevel, weatherRec) => {
    if (weatherRec.status === 'excellent' && crowdLevel < 50) {
      return `${weatherRec.icon} ${weatherRec.message} + Perfect crowd levels!`;
    } else if (weatherRec.status === 'excellent' && crowdLevel > 80) {
      return `${weatherRec.icon} ${weatherRec.message} but very crowded due to perfect conditions`;
    } else if (weatherRec.status === 'poor') {
      return `⚠️ ${weatherRec.message} - Consider alternatives`;
    } else {
      return `${weatherRec.icon} ${weatherRec.message}`;
    }
  };
  ...
  export {
    ...
    WeatherSmartRecommendation,
    integrateCrowdAndWeather
  };